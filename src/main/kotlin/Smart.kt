...
internal class Smart {
    ...
    class Philosopher(val leftFork: Fork, val rightFork: Fork) {
        ...
        suspend fun tryToEat() = coroutineScope {
            launch {
                var count = 0
                val firstFork: Fork = if (leftFork.num < rightFork.num) {
                    leftFork
                } else {
                    rightFork
                }
                val secondFork: Fork = if (leftFork.num > rightFork.num) {
                    leftFork
                } else {
                    rightFork
                }
                while (!take(firstFork)) {
                    delay(4000L)
                    count += 1
